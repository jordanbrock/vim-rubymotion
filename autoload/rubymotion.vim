" autoload/rubymotion.vim
" Author: Jordan Brock <http://github.com/jordanbrock>
"
" Install this file as autoload/rubymotion.vim
"

if exists('g:autoloaded_rubymotion') || &cp
  finish
endif
let g:autoloaded_rubymotion = '1.0'
...
function! rubymotion#lencmp(i1, i2) abort
...
function! s:push_chdir(...)
  ...
  if exists("b:rubymotion_root") && (a:0 ? getcwd() !=# rubymotion#app().path() : !s:startswith(getcwd(), rubymotion#app().path()))
    let chdir = exists("*haslocaldir") && haslocaldir() ? "lchdir " : "chdir "
    call add(s:command_stack,chdir.s:escarg(getcwd()))
    exe chdir.s:escarg(rubymotion#app().path())
  else
...
function! s:endof(lnum)
  return rubymotion#buffer().end_of(a:lnum)
endfunction
...
function! s:lastopeningline(pattern,limit,start)
  return rubymotion#buffer().last_opening_line(a:start,a:pattern,a:limit)
endfunction
...
function! s:lastmethodline(start)
  return rubymotion#buffer().last_method_line(a:start)
endfunction
...
function! s:lastmethod(...)
  return rubymotion#buffer().last_method(a:0 ? a:1 : line("."))
endfunction
...
function! s:format()
  return rubymotion#buffer().format(line('.'))
endfunction
...
function! s:readable_find_affinity() dict abort
  ...
  for pattern in reverse(sort(filter(keys(all), 'v:val =~# "^[^*{}]*\\*[^*{}]*$"'), s:function('rubymotion#lencmp')))
...
function! s:controller(...)
  return rubymotion#buffer().controller_name(a:0 ? a:1 : 0)
endfunction

function! s:readable_controller_name(...) dict abort
  let f = self.name()
  if has_key(self,'getvar') && self.getvar('rubymotion_controller') != ''
    return self.getvar('rubymotion_controller')
  endif
  let [affinity, root] = self.find_affinity()
  if affinity ==# 'controller'
    return root
  elseif affinity ==# 'resource'
    return rubymotion#pluralize(root)
  endif
  ...
  elseif a:0 && a:1
    return rubymotion#pluralize(self.model_name())
  endif
  return ""
endfunction

function! s:model(...)
  return rubymotion#buffer().model_name(a:0 ? a:1 : 0)
endfunction

function! s:readable_model_name(...) dict abort
  let f = self.name()
  if has_key(self,'getvar') && self.getvar('rubymotion_model') != ''
    return self.getvar('rubymotion_model')
  endif
  let [affinity, root] = self.find_affinity()
  if affinity ==# 'model'
    return root
  elseif affinity ==# 'collection'
    return rubymotion#singularize(root)
  endif
  ...
  elseif f =~ '\<\%(test\|spec\)/\%(fixtures\|factories\|fabricators\)/.*\.\w\+$'
    return rubymotion#singularize(s:sub(f,'.*<%(test|spec)/\w+/(.*)\.\w+$','\1'))
  elseif a:0 && a:1
    return rubymotion#singularize(self.controller_name())
  endif
  return ""
endfunction
...
function! s:environment()
  if exists('$rubymotion_ENV')
    return $rubymotion_ENV
...
function! s:Complete_environments(...)
  return s:completion_filter(rubymotion#app().environments(),a:0 ? a:1 : "")
endfunction
...
function! s:debug(str)
  if exists("g:rubymotion_debug") && g:rubymotion_debug
    echohl Debug
    echomsg a:str
    echohl None
  endif
...
function! rubymotion#underscore(str)
...
function! rubymotion#camelize(str)
  let str = s:gsub(a:str,'/(.=)','::\u\1')
  let str = s:gsub(str,'%([_-]|<)(.)','\u\1')
  return str
endfunction

function! rubymotion#singularize(word)
  " Probably not worth it to be as comprehensive as rubymotion but we can
...
function! rubymotion#pluralize(word)
  let word = a:word
  if word == ''
    return word
  endif
...
function! rubymotion#app(...)
  let root = a:0 ? a:1 : get(b:, 'rubymotion_root', '')
  if !empty(root)
...
function! rubymotion#buffer(...)
  return extend(extend({'#': bufnr(a:0 ? a:1 : '%')},s:buffer_prototype,'keep'),s:readable_prototype,'keep')
  endif
endfunction

function! s:buffer_app() dict abort
  if self.getvar('rubymotion_root') != ''
    return rubymotion#app(self.getvar('rubymotion_root'))
  else
    throw 'Not in a rubymotion app'
  endif
endfunction
...
function! rubymotionRevision()
  return 1000*matchstr(g:autoloaded_rubymotion,'^\d\+')+matchstr(g:autoloaded_rubymotion,'[1-9]\d*$')
endfunction

function! rubymotionRoot()
  if exists("b:rubymotion_root")
    return b:rubymotion_root
  else
    return ""
  endif
endfunction
...
function! s:buffer_name() dict abort
  ...
  if s:startswith(tolower(f),s:gsub(tolower(app.path()),'\\ @!','/')) || f == ""
    ...
    if !exists("s:path_warn") && &verbose
      let s:path_warn = 1
      call s:warn("File ".f." does not appear to be under the rubymotion root ".self.app().path().". Please report to the rubymotion.vim author!")
    endif
    return f
  endif
endfunction

function! rubymotionFilePath()
  if !exists("b:rubymotion_root")
    return ""
  else
    return rubymotion#buffer().name()
  endif
endfunction

function! rubymotionFile()
  return rubymotionFilePath()
endfunction

function! rubymotionFileType()
  if !exists("b:rubymotion_root")
    return ""
  else
    return rubymotion#buffer().type_name()
  endif
endfunction
...
function! s:readable_calculate_file_type() dict abort
  ...
  if f == ""
    let r = f
  elseif nr > 0 && getbufvar(nr,'rubymotion_file_type') != ''
    return getbufvar(nr,'rubymotion_file_type')
...
function! s:buffer_type_name(...) dict abort
  let type = getbufvar(self.number(),'rubymotion_cached_file_type')
  if type == ''
...
function! s:app_stylesheet_suffix() dict abort
  if self.cache.needs('stylesheet_suffix')
    let default = self.has_gem('sass-rubymotion') ? '.css.scss' : '.css'
    let candidates = map(filter(
...
function! s:app_has(feature) dict
  ...
  if !has_key(features,a:feature)
    ...
    let features[a:feature] = rubymotion#app().has_path(path)
  endif
...
function! s:app_prepare_rubymotion_command(cmd) dict abort
  if self.has_path('.zeus.sock') && a:cmd =~# '^\%(console\|dbconsole\|destroy\|generate\|server\|runner\)\>'
    return 'zeus '.a:cmd
  elseif self.has_path('script/rubymotion')
    let cmd = 'script/rubymotion '.a:cmd
  elseif self.has_path('script/' . matchstr(a:cmd, '\w\+'))
    let cmd = 'script/'.a:cmd
  elseif self.has_path('bin/rubymotion')
    let cmd = 'bin/rubymotion '.a:cmd
  elseif self.has('bundler')
    return 'bundle exec rubymotion ' . a:cmd
  else
    return 'rubymotion '.a:cmd
  endif
  return self.ruby_script_command(cmd)
endfunction

function! s:app_start_rubymotion_command(cmd, ...) dict abort
  let cmd = s:esccmd(self.prepare_rubymotion_command(a:cmd))
  ...
  try
    if exists(':Start') == 2
      exe 'Start'.(a:0 && a:1 ? '!' : '').' -title=rubymotion\ '.title.' '.cmd
    elseif has("win32")
      exe "!start ".cmd
    else
      exe "!".cmd
...
function! s:app_execute_rubymotion_command(cmd) dict abort
  call s:push_chdir(1)
  try
    exe '!'.s:esccmd(self.prepare_rubymotion_command(a:cmd))
  finally
    call s:pop_command()
  endtry
  return ''
...
call s:add_methods('app', ['ruby_command','ruby_script_command','prepare_rubymotion_command','execute_rubymotion_command','start_rubymotion_command','lightweight_ruby_eval','eval'])
...
function! s:BufCommands()
  ...
  command! -buffer -bar -nargs=? -bang -count -complete=customlist,rubymotion#complete_rake Rake    :call s:Rake(<bang>0,!<count> && <line1> ? -1 : <count>,<q-args>)
  ...
  command! -buffer -bar -nargs=0 Rtags       :execute rubymotion#app().tags_command()
  command! -buffer -bar -nargs=0 Ctags       :execute rubymotion#app().tags_command()
  command! -buffer -bar -nargs=0 -bang Rrefresh :if <bang>0|unlet! g:autoloaded_rubymotion|source `=s:file`|endif|call s:Refresh(<bang>0)
  ...
  let ext = expand("%:e")
  if rubymotionFilePath() =~ '\<app/views/'
  ...
  elseif rubymotion#buffer().name() =~# '^app/helpers/.*\.rb$'
  ...
  elseif rubymotion#buffer().name() =~# '^app/\w\+/.*\.rb$'
    command! -buffer -bar -bang -nargs=1 -range Rextract :<line1>,<line2>call s:RubyExtract(<bang>0, matchstr(rubymotion#buffer().name(), '^app/\w\+/').'concerns', ['  extend ActiveSupport::Concern', ''], <f-args>)
    command! -buffer -bar -bang -nargs=1 -range Extract  :<line1>,<line2>call s:RubyExtract(<bang>0, matchstr(rubymotion#buffer().name(), '^app/\w\+/').'concerns', ['  extend ActiveSupport::Concern', ''], <f-args>)
  endif
  if rubymotionFilePath() =~ '\<db/migrate/.*\.rb$'
...
function! s:Log(bang,arg)
  ...
  if a:bang
    ...
    if exists(":Tail") == 2
      Tail  `=rubymotion#app().path(lf)`
    else
      pedit `=rubymotion#app().path(lf)`
    endif
  endif
endfunction

function! rubymotion#new_app_command(bang,...) abort
  if !a:0 && a:bang
    echo "rubymotion.vim ".g:autoloaded_rubymotion
  elseif !a:0 || a:1 !=# 'new'
    return 'echoerr '.string('Usage: rubymotion new <path>')
  endif
  ...
  try
    ...
    exe '!rubymotion' join(map(copy(args),'s:rquote(v:val)'),' ') pipe
  catch /^Vim:Interrupt/
  endtry
...
function! s:app_tags_command() dict abort
  ...
  try
    ...
    if self.has_path('.ctags')
      ...
      let args = s:split(get(g:, 'rubymotion_ctags_arguments', '--languages=ruby'))
    endif
    exe '!'.cmd.' -R '.join(args,' ')
  finally
...
function! s:Refresh(bang)
  if exists("g:rubycomplete_rubymotion") && g:rubycomplete_rubymotion && has("ruby") && exists('g:rubycomplete_completions')
  ...
  let _ = rubymotion#app().cache.clear()
  silent doautocmd User BufLeaverubymotion
  if a:bang
  ...
  let max = bufnr('$')
  while i <= max
    let rr = getbufvar(i,"rubymotion_root")
    if rr != ""
      call setbufvar(i,"rubymotion_refresh",1)
    endif
    let i += 1
  endwhile
  silent doautocmd User BufEnterrubymotion
endfunction

function! s:RefreshBuffer()
  if exists("b:rubymotion_refresh") && b:rubymotion_refresh
    let b:rubymotion_refresh = 0
    let &filetype = &filetype
    unlet! b:rubymotion_refresh
  endif
endfunction
...
let g:rubymotion#rake_errorformat = '%D(in\ %f),'
      \.'%\\s%#from\ %f:%l:%m,'
      \.'%\\s%#from\ %f:%l:,'
      \.'%\\s#{rubymotion_ROOT}/%f:%l:\ %#%m,'
      \.'%\\s%##\ %f:%l:%m,'
      \.'%\\s%##\ %f:%l,'
      \.'%\\s%#[%f:%l:\ %#%m,'
      \.'%\\s%#%f:%l:\ %#%m,'
...
function! s:Rake(bang,lnum,arg)
  ...
  try
    ...
    let &l:makeprg = rubymotion#app().rake_command()
    let &l:errorformat = g:rubymotion#rake_errorformat
    let arg = a:arg
    ...
    if arg == ''
      let arg = rubymotion#buffer().default_rake_task(lnum)
    endif
...
function! rubymotion#complete_rake(A,L,P)
  return s:completion_filter(rubymotion#app().rake_tasks(), a:A, ':')
endfunction
...
function! s:readable_preview_urls(lnum) dict abort
  ...
  while start < self.line_count() && self.getline(start) =~ '^\s*\%(\%(-\=\|<%\)#.*\)\=$'
    ...
    let start += 1
  endwhile
  if has_key(self,'getvar') && self.getvar('rubymotion_preview') != ''
    let urls += [self.getvar('rubymotion_preview')]
  endif
...
function! s:app_server_binding() dict abort
  ...
  if pid
    ...
    let binding = rubymotion#get_binding_for(pid)
...
function! s:Preview(bang, lnum, uri) abort
  let binding = rubymotion#app().server_binding()
  if empty(binding)
    let binding = '0.0.0.0:3000'
  endif
  let binding = s:sub(binding, '^0\.0\.0\.0>|^127\.0\.0\.1>', 'localhost')
  let uri = empty(a:uri) ? get(rubymotion#buffer().preview_urls(a:lnum),0,'') : a:uri
  if uri =~ '://'
    "
  elseif uri =~# '^[[:alnum:]-]\+\.'
  ...
  if (exists(':OpenURL') == 2) && !a:bang
    ...
    let root = rubymotion#app().path()
    wincmd w
    let b:rubymotion_root = root
    if &filetype ==# ''
      if uri =~ '\.css$'
...
function! s:Complete_preview(A,L,P)
  return rubymotion#buffer().preview_urls(a:L =~ '^\d' ? matchstr(a:L,'^\d\+') : line('.'))
endfunction
...
function! s:BufScriptWrappers()
  command! -buffer -bang -bar -nargs=* -complete=customlist,s:Complete_script   Rscript       :execute empty(<q-args>) ? rubymotion#app().script_command(<bang>0, 'console') ? rubymotion#app().script_command(<bang>0,<f-args>)
  command! -buffer -bang -bar -nargs=* -complete=customlist,s:Complete_environments Console   :rubymotion<bang> console <args>
  command! -buffer -bang -bar -nargs=* -complete=customlist,s:Complete_script   rubymotion         :execute rubymotion#app().script_command(<bang>0,<f-args>)
  command! -buffer -bang -bar -nargs=* -complete=customlist,s:Complete_generate Rgenerate     :execute rubymotion#app().generator_command(<bang>0,'generate',<f-args>)
  command! -buffer -bang -bar -nargs=* -complete=customlist,s:Complete_generate Generate      :execute rubymotion#app().generator_command(<bang>0,'generate',<f-args>)
  command! -buffer -bar -nargs=*       -complete=customlist,s:Complete_destroy  Rdestroy      :execute rubymotion#app().generator_command(1,'destroy',<f-args>)
  command! -buffer -bar -nargs=*       -complete=customlist,s:Complete_destroy  Destroy       :execute rubymotion#app().generator_command(1,'destroy',<f-args>)
  command! -buffer -bar -nargs=? -bang -complete=customlist,s:Complete_server   Rserver       :execute rubymotion#app().server_command(<bang>0,<q-args>)
  command! -buffer -bar -nargs=? -bang -complete=customlist,s:Complete_server   Server        :execute rubymotion#app().server_command(<bang>0,<q-args>)
  command! -buffer -bang -nargs=? -range=0 -complete=customlist,s:Complete_edit Rrunner       :execute rubymotion#buffer().runner_command(<bang>0, <count>?<line1>:0, <q-args>)
  command! -buffer -bang -nargs=? -range=0 -complete=customlist,s:Complete_edit Runner        :execute rubymotion#buffer().runner_command(<bang>0, <count>?<line1>:0, <q-args>)
  command! -buffer       -nargs=1 -range=0 -complete=customlist,s:Complete_ruby Rp            :execute rubymotion#app().output_command(<count>==<line2>?<count>:-1, 'p begin '.<q-args>.' end')
  command! -buffer       -nargs=1 -range=0 -complete=customlist,s:Complete_ruby Rpp           :execute rubymotion#app().output_command(<count>==<line2>?<count>:-1, 'require %{pp}; pp begin '.<q-args>.' end')
endfunction

function! s:app_generators() dict abort
  if self.cache.needs('generators')
    let paths = [self.path('vendor/plugins/*'), self.path('lib'), expand("~/.rubymotion")]
    if !empty(self.gems())
      let gems = values(self.gems())
      let paths += map(gems, 'v:val . "/lib/rubymotion"')
    ...
    let builtin += map(filter(copy(generators), 'v:val =~# "^rubymotion:"'), 'v:val[6:-1]')
    call filter(generators,'v:val !~# "^rubymotion:"')
...
function! s:app_script_command(bang,...) dict
  let msg = "rubymotion.vim ".g:autoloaded_rubymotion
  if a:0 == 0 && a:bang && rubymotion#buffer().type_name() == ''
    echo msg." (rubymotion)"
    return
  elseif a:0 == 0 && a:bang
    echo msg." (rubymotion-".rubymotion#buffer().type_name().")"
    return
  endif
  let str = join(map(copy(a:000), 's:rquote(v:val)'), ' ')
  if a:bang || str =~# '^\%(c\|console\|db\|dbconsole\|s\|server\)\>'
    return self.start_rubymotion_command(str, a:bang)
  else
    return self.execute_rubymotion_command(str)
  endif
endfunction
...
function! s:readable_runner_command(bang, count, arg) dict abort
  ...
  try
    ...
    if compiler ==# 'ruby'
      let &l:makeprg = self.app().prepare_rubymotion_command('runner')
      let extra = ''
...
function! s:app_output_command(count, code) dict
  let str = self.prepare_rubymotion_command('runner '.s:rquote(a:code))
  call s:push_chdir(1)
  try
...
function! rubymotion#get_binding_for(pid)
  if empty(a:pid)
    return ''
  endif
  if has('win32')
...
function! s:app_server_command(bang,arg) dict
  if a:arg =~# '--help'
    call self.execute_rubymotion_command('server '.a:arg)
    return ''
  endif
  let pidfile = self.path('tmp/pids/server.pid')
  ...
  if (exists(':Start') == 2) || has('win32')
    call self.start_rubymotion_command('server '.a:arg, 1)
  else
    call self.execute_rubymotion_command('server '.a:arg.' -d')
  endif
  return ''
endfunction
...
function! s:app_generator_command(bang,...) dict
  ...
  let old_errorformat = &l:errorformat
  try
    let &l:makeprg = self.prepare_rubymotion_command(cmd)
    let &l:errorformat = s:efm_generate
    call s:push_chdir(1)
    noautocmd make!
  finally
...
function! s:Complete_script(ArgLead,CmdLine,P)
  ...
  elseif cmd =~# '^\%(generate\|destroy\)\s\+'.a:ArgLead.'$'
    return s:completion_filter(rubymotion#app().generators(),a:ArgLead)
  elseif cmd =~# '^\%(generate\|destroy\)\s\+\w\+\s\+'.a:ArgLead.'$'
    ...
    elseif target ==# 'generator'
      return s:completion_filter(map(rubymotion#app().relglob('lib/generators/','*'),'s:sub(v:val,"/$","")'), a:ArgLead)
    elseif target ==# 'helper'
      return s:autocamelize(rubymotion#app().relglob('app/helpers/','**/*','_helper.rb'),a:ArgLead)
    elseif target ==# 'integration_test' || target ==# 'integration_spec' || target ==# 'feature'
      return s:autocamelize(
            \ rubymotion#app().relglob('test/integration/','**/*','_test.rb') +
            \ rubymotion#app().relglob('spec/features/', '**/*', '_spec.rb') +
            \ rubymotion#app().relglob('spec/requests/', '**/*', '_spec.rb') +
            \ rubymotion#app().relglob('features/', '**/*', '.feature'), a:ArgLead)
    ...
    elseif target ==# 'mailer'
      ...
      return s:completion_filter(rubymotion#app().relglob("app/mailers/","**/*",".rb"),a:ArgLead)
    elseif target =~# '^\w*\%(model\|resource\)$' || target =~# '\w*scaffold\%(_controller\)\=$'
      return s:completion_filter(rubymotion#app().relglob('app/models/','**/*','.rb'), a:ArgLead)
    else
      return []
    endif
  elseif cmd =~# '^\%(generate\|destroy\)\s\+scaffold\s\+\w\+\s\+'.a:ArgLead.'$'
    return filter(s:controllerList(a:ArgLead,"",""),'v:val !=# "application"')
    return s:completion_filter(rubymotion#app().environments())
  elseif cmd =~# '^\%(console\)\s\+\(--\=\w\+\s\+\)\='.a:ArgLead."$"
    return s:completion_filter(rubymotion#app().environments()+["-s","--sandbox"],a:ArgLead)
  elseif cmd =~# '^\%(server\)\s\+.*-e\s\+'.a:ArgLead."$"
    return s:completion_filter(rubymotion#app().environments(),a:ArgLead)
  elseif cmd =~# '^\%(server\)\s\+'
    if a:ArgLead =~# '^--environment='
      return s:completion_filter(map(copy(rubymotion#app().environments()),'"--environment=".v:val'),a:ArgLead)
    else
...
function! s:Complete_ruby(A,L,P)
  return s:completion_filter(rubymotion#app().user_classes()+["ActiveRecord::Base"],a:A)
endfunction
...
function! s:BufNavCommands()
  command! -buffer -bar -nargs=? -complete=customlist,s:Complete_cd Cd    :cd `=rubymotion#app().path(<q-args>)`
  command! -buffer -bar -nargs=? -complete=customlist,s:Complete_cd Lcd  :lcd `=rubymotion#app().path(<q-args>)`
  command! -buffer -bar -nargs=? -complete=customlist,s:Complete_cd Rcd   :cd `=rubymotion#app().path(<q-args>)`
  command! -buffer -bar -nargs=? -complete=customlist,s:Complete_cd Rlcd :lcd `=rubymotion#app().path(<q-args>)`
...
function! s:Find(count,cmd,...)
  ...
  if a:0
    ...
    if file != ""
      let file = s:rubymotionIncludefind(file)
    endif
  else
    let file = s:rubymotionFind()
    let tail = ""
  endif
...
function! s:Complete_find(ArgLead, CmdLine, CursorPos)
  ...
  let seen = {}
  for path in paths
    if s:startswith(path,rubymotion#app().path()) && path !~ '[][*]'
      let path = path[strlen(rubymotion#app().path()) + 1 : ]
      for file in rubymotion#app().relglob(path == '' ? '' : path.'/',s:fuzzyglob(rubymotion#underscore(a:ArgLead)), a:ArgLead =~# '\u' ? '.rb' : '')
        let seen[file] = 1
      endfor
    endif
  endfor
...
function! s:Complete_edit(ArgLead, CmdLine, CursorPos)
  return s:completion_filter(rubymotion#app().relglob("",s:fuzzyglob(a:ArgLead)),a:ArgLead)
endfunction

function! s:Complete_cd(ArgLead, CmdLine, CursorPos)
  let all = rubymotion#app().relglob("",a:ArgLead."*")
  call filter(all,'v:val =~ "/$"')
  return filter(all,'s:startswith(v:val,a:ArgLead)')
endfunction

function! rubymotionIncludeexpr()
  " Is this foolproof?
  if mode() =~ '[iR]' || expand("<cfile>") != v:fname
    return s:rubymotionIncludefind(v:fname)
  else
    return s:rubymotionIncludefind(v:fname,1)
  endif
endfunction
...
function! s:rubymotionFind()
  if filereadable(expand("<cfile>"))
    return expand("<cfile>")
  endif

  " UGH
  let buffer = rubymotion#buffer()
  let format = s:format()
  ...
  let res = rubymotion#underscore(s:findit('\v\s*<%(include|extend)\(=\s*<([[:alnum:]_:]+)>','\1'))
  ...
  let res = rubymotion#singularize(s:findamethod('has_many\|has_and_belongs_to_many\|embeds_many','\1'))
  if res != ""|return res.".rb"|endif

  let res = rubymotion#singularize(s:findamethod('create_table\|change_table\|drop_table\|rename_table\|\%(add\|remove\)_\%(column\|index\|timestamps\|reference\|belongs_to\)\|rename_column\|remove_columns\|rename_index','\1'))
  if res != ""|return res.".rb"|endif

  let res = rubymotion#singularize(s:findasymbol('through','\1'))
  if res != ""|return res.".rb"|endif

  let res = s:findamethod('fixtures','fixtures/\1')
  if res != ""
    return rubymotionFilePath() =~ '\<spec/' ? 'spec/'.res : res
  endif
  ...
  if res != ""|return rubymotion#pluralize(res)."_controller.rb"|endif
  ...
  endtry
  let res = s:rubymotionIncludefind(cfile,1)
  return res
endfunction
...
function! s:rubymotionIncludefind(str,...)
  ...
  let str = rubymotion#underscore(str)
  let fpat = '\(\s*\%("\f*"\|:\f*\|'."'\\f*'".'\)\s*,\s*\)*'
  if a:str =~# '\u'
    " Classes should always be in .rb files
    let str .= '.rb'
  elseif line =~# ':partial\s*=>\s*' || (line =~# ':layout\s*=>\s*' && !rubymotion#buffer().type_name('controller', 'mailer'))
    let str = s:sub(str,'[^/]+$','_&')
  ...
  elseif line =~# '\<has_\(and_belongs_to_\)\=many\s*(\=\s*'
    let str = rubymotion#singularize(str).'.rb'
  elseif line =~# '\<def\s\+' && expand("%:t") =~# '_controller\.rb'
    let str = s:findview(str)
  elseif str =~# '_\%(path\|url\)$' || (line =~# ':as\s*=>\s*$' && rubymotion#buffer().type_name('config-routes'))
    if line !~# ':as\s*=>\s*$'
      let str = s:sub(str,'_%(path|url)$','')
      let str = s:sub(str,'^hash_for_','')
    endif
    let file = rubymotion#app().named_route_file(str)
    if file == ""
      let str = s:sub(str,'^formatted_','')
      if str =~# '^\%(new\|edit\)_'
        let str = s:sub(rubymotion#pluralize(str),'^(new|edit)_(.*)','\2_controller.rb#\1')
      elseif str ==# rubymotion#singularize(str)
        ...
        let str = rubymotion#pluralize(str).'_controller.rb#show'
      else
  ...
  elseif str !~ '/'
    ...
    let str = rubymotion#singularize(str)
...
function! s:app_commands() dict abort
  ...
  for pattern in reverse(sort(keys(all), function('rubymotion#lencmp')))
...
function! s:autocamelize(files,test)
  if a:test =~# '^\u'
    return s:completion_filter(map(copy(a:files),'rubymotion#camelize(v:val)'),a:test)
  else
...
function! s:relglob(...)
  return join(call(rubymotion#app().relglob,a:000,rubymotion#app()),"\n")
endfunction

function! s:controllerList(A,L,P)
  let con = rubymotion#app().relglob("app/controllers/","**/*",".rb")
...
function! s:viewList(A,L,P)
  ...
  let top = rubymotion#app().relglob("app/views/",s:fuzzyglob(a:A))
  call filter(top,'v:val !~# "\\~$"')
  if c != '' && a:A !~ '/'
    let local = rubymotion#app().relglob("app/views/".c."/","*.*[^~]")
...
function! s:layoutList(A,L,P)
  return s:completion_filter(rubymotion#app().relglob("app/views/layouts/","*"),a:A)
endfunction

function! s:stylesheetList(A,L,P)
  let list = rubymotion#app().relglob('app/assets/stylesheets/','**/*.*','')
  call map(list,'s:sub(v:val,"\\..*$","")')
  let list += rubymotion#app().relglob('public/stylesheets/','**/*','.css')
  if rubymotion#app().has('sass')
    call extend(list,rubymotion#app().relglob('public/stylesheets/sass/','**/*','.s?ss'))
    call s:uniq(list)
  endif
...
function! s:javascriptList(A,L,P)
  let list = rubymotion#app().relglob('app/assets/javascripts/','**/*.*','')
  call map(list,'s:sub(v:val,"\\.js\\..*|\\.\\w+$","")')
  let list += rubymotion#app().relglob("public/javascripts/","**/*",".js")
  return s:completion_filter(list,a:A)
endfunction

function! s:fixturesList(A,L,P)
  return s:completion_filter(
        \ rubymotion#app().relglob('test/fixtures/', '**/*') +
        \ rubymotion#app().relglob('spec/fixtures/', '**/*') +
        \ rubymotion#app().relglob('test/factories/', '**/*') +
        \ rubymotion#app().relglob('spec/factories/', '**/*'),
        \ a:A)
endfunction

function! s:localeList(A,L,P)
  return s:completion_filter(rubymotion#app().relglob("config/locales/","**/*"),a:A)
endfunction

function! s:migrationList(A,L,P)
  if a:A =~ '^\d'
    let migrations = rubymotion#app().relglob("db/migrate/",a:A."[0-9_]*",".rb")
    return map(migrations,'matchstr(v:val,"^[0-9]*")')
  else
    let migrations = rubymotion#app().relglob("db/migrate/","[0-9]*[0-9]_*",".rb")
...
function! s:schemaList(A,L,P)
  let tables = s:readfile(rubymotion#app().path('db/schema.rb'))
...
function! s:specList(A,L,P)
  return s:completion_filter(rubymotion#app().relglob("spec/","**/*","_spec.rb"),a:A)
endfunction
    ...
    if arg =~# '^[a-z]\+$'
      for prefix in ['E', 'S', 'V', 'T', 'D', 'R', 'RE', 'RS', 'RV', 'RT', 'RD']
        exe 'command! -buffer -bar -bang -nargs=* ' .
              \ (prefix =~# 'D' ? '-range=0 ' : '') .
              \ prefix . arg . ' :echoerr ' .
              \ string(':Rnavcommand has been removed.  See :help rubymotion-projections')
      endfor
      break
    endif
  endfor
...
function! s:CommandList(A,L,P)
  ...
  for projection in s:last_projections
    ...
    let results = rubymotion#app().relglob(prefix, '**/*', suffix)
    if suffix =~# '\.rb$' && a:A =~# '^\u'
      let matches += map(results, 'rubymotion#camelize(v:val)')
    else
      let matches += results
    endif
  endfor
...
function! s:CommandEdit(cmd, name, projections, ...)
  if a:0 && a:1 == "&"
    ...
    return rubymotion#buffer().open_command(a:cmd, a:0 ? a:1 : '', a:name, a:projections)
  endif
endfunction
...
function! s:app_migration(file) dict
  ...
  elseif arg == ''
    ...
    let glob = '*'.rubymotion#underscore(arg).'*rb'
  endif
...
function! s:migrationEdit(cmd,...)
  ...
  if arg =~# '!'
    ...
    let template = 'class ' . rubymotion#camelize(matchstr(arg, '[^!]*')) . " < ActiveRecord::Migration\nend"
    return rubymotion#buffer().open_command(a:cmd, strftime('%Y%m%d%H%M%S', ts - offset).'_'.arg, 'migration',
          \ [{'pattern': 'db/migrate/*.rb', 'template': template}])
  endif
  let migr = arg == "." ? "db/migrate" : rubymotion#app().migration(arg)
  if migr != ''
...
function! s:schemaEdit(cmd,...)
  ...
  if !rubymotion#app().has_file('db/schema.rb')
    if rubymotion#app().has_file('db/structure.sql')
      let schema = 'db/structure.sql'
    elseif rubymotion#app().has_file('db/'.s:environment().'_structure.sql')
...
function! s:fixturesEdit(cmd,...)
  if a:0
    let c = rubymotion#underscore(a:1)
  else
    let c = rubymotion#pluralize(s:model(1))
  endif
  if c == ""
    return s:error("E471: Argument required")
  endif
  ...
  let file = get(filter(copy(dirs), 'isdirectory(rubymotion#app().path(v:val))'), 0, dirs[0]).'/'.c.e
  if file =~ '\.\w\+$' && rubymotion#app().find_file(c.e, dirs) ==# ''
    return s:edit(a:cmd,file)
  else
    return s:findedit(a:cmd, rubymotion#app().find_file(c.e, dirs, ['.yml', '.csv', '.rb'], file))
  endif
endfunction

function! s:localeEdit(cmd,...)
  let c = a:0 ? a:1 : rubymotion#app().default_locale()
  if c =~# '\.'
    return s:edit(a:cmd,rubymotion#app().find_file(c,'config/locales',[],'config/locales/'.c))
  else
    return rubymotion#buffer().open_command(a:cmd, c, 'locale',
...
function! s:findview(name)
  return rubymotion#buffer().resolve_view(a:name, line('.'))
endfunction

function! s:findlayout(name)
  return rubymotion#buffer().resolve_layout(a:name, line('.'))
endfunction

function! s:viewEdit(cmd, ...) abort
  if a:0 && a:1 =~ '^[^!#:]'
    let view = matchstr(a:1,'[^!#:]*')
  elseif rubymotion#buffer().type_name('controller','mailer')
  ...
  let found = rubymotion#buffer().resolve_view(view, line('.'))
  ...
  elseif a:0 && a:1 =~# '!'
    ...
    if !rubymotion#app().has_path(fnamemodify(file, ':h'))
      call mkdir(rubymotion#app().path(fnamemodify(file, ':h')), 'p')
    endif
...
function! s:controllerEdit(cmd,...)
  ...
  if a:0 == 0
    ...
    if rubymotion#buffer().type_name() =~# '^view\%(-layout\|-partial\)\@!'
      let jump = '#'.expand('%:t:r')
    else
      let jump = ''
    endif
  ...
  if rubymotion#app().has_file("app/controllers/".controller."_controller.rb") || !rubymotion#app().has_file("app/controllers/".controller.".rb")
    let template = "class %SController < ApplicationController\nend"
    let suffix = "_controller".suffix
  endif
  return rubymotion#buffer().open_command(a:cmd, controller . jump, 'controller',
        \ [{'template': template, 'pattern': 'app/controllers/*'.suffix}])
endfunction

function! s:stylesheetEdit(cmd,...)
  let name = a:0 ? a:1 : s:controller(1)
  if rubymotion#app().has('sass') && rubymotion#app().has_file('public/stylesheets/sass/'.name.'.sass')
    return s:LegacyCommandEdit(a:cmd,name,"public/stylesheets/sass/",".sass")
  elseif rubymotion#app().has('sass') && rubymotion#app().has_file('public/stylesheets/sass/'.name.'.scss')
    return s:LegacyCommandEdit(a:cmd,name,"public/stylesheets/sass/",".scss")
  elseif rubymotion#app().has('lesscss') && rubymotion#app().has_file('app/stylesheets/'.name.'.less')
    return s:LegacyCommandEdit(a:cmd,name,"app/stylesheets/",".less")
  else
    let types = rubymotion#app().relglob('app/assets/stylesheets/'.name,'.*','')
    if !empty(types)
      return s:LegacyCommandEdit(a:cmd,name,'app/assets/stylesheets/',types[0])
    elseif !isdirectory(rubymotion#app().path('app/assets/stylesheets'))
      return s:LegacyCommandEdit(a:cmd,name,'public/stylesheets/','.css')
    else
      return s:LegacyCommandEdit(a:cmd,name,'app/assets/stylesheets/',rubymotion#app().stylesheet_suffix())
    endif
  endif
endfunction
...
function! s:javascriptEdit(cmd,...)
  let name = a:0 ? a:1 : s:controller(1)
  if rubymotion#app().has('coffee') && rubymotion#app().has_file('app/scripts/'.name.'.coffee')
    return s:LegacyCommandEdit(a:cmd,name,'app/scripts/','.coffee')
  elseif rubymotion#app().has('coffee') && rubymotion#app().has_file('app/scripts/'.name.'.js')
    return s:LegacyCommandEdit(a:cmd,name,'app/scripts/','.js')
  else
    let types = rubymotion#app().relglob('app/assets/javascripts/'.name,'.*','')
    if !empty(types)
      return s:LegacyCommandEdit(a:cmd,name,'app/assets/javascripts/',types[0])
    elseif !isdirectory(rubymotion#app().path('app/assets/javascripts'))
      return s:LegacyCommandEdit(a:cmd,name,'public/javascripts/','.js')
    elseif rubymotion#app().has_gem('coffee-rubymotion')
...
function! s:specEdit(cmd,...) abort
  let describe = s:sub(s:sub(rubymotion#camelize(a:0 ? a:1 : ''), '^[^:]*::', ''), '!.*', '')
  return rubymotion#buffer().open_command(a:cmd, a:0 ? a:1 : '', 'spec', [
...
function! s:readable_open_command(cmd, argument, name, projections) dict abort
  ...
  for projection in a:projections
    ...
    if self.app().has_path(prefix)
      let relative = prefix . (suffix =~# '\.rb$' ? rubymotion#underscore(root) : root) . suffix
      ...
      let ph = {
            ...
            \ 'S': rubymotion#camelize(root),
            \ 'h': toupper(root[0]) . tr(rubymotion#underscore(root), '_', ' ')[1:-1]}
...
function! s:findedit(cmd,files,...) abort
  ...
  if len(files) == 1
    ...
    let file = get(filter(copy(files),'rubymotion#app().has_file(s:sub(v:val,"#.*|:\\d*$",""))'),0,get(files,0,''))
  endif
  if file =~ '[#!]\|:\d*\%(:in\)\=$'
  ...
  if file == ''
    let testcmd = "edit"
  elseif rubymotion#app().has_path(file.'/')
    let arg = file == "." ? rubymotion#app().path() : rubymotion#app().path(file)
    let testcmd = s:editcmdfor(cmd).' '.(a:0 ? a:1 . ' ' : '').s:escarg(arg)
    exe testcmd
    return ''
  elseif rubymotion#app().path() =~ '://' || cmd =~ 'edit' || cmd =~ 'split'
    if file !~ '^/' && file !~ '^\w:' && file !~ '://'
      let file = s:escarg(rubymotion#app().path(file))
    endif
...
function! s:edit(cmd,file,...)
  ...
  let file = a:file
  if file !~ '^/' && file !~ '^\w:' && file !~ '://'
    exe cmd."`=fnamemodify(rubymotion#app().path(file),':.')`"
  else
    exe cmd.file
  endif
  return ''
...
function! s:Alternate(cmd,line1,line2,count,...)
  if a:0
    ...
    let file = get(b:, a:count ? 'rubymotion_related' : 'rubymotion_alternate')
    if empty(file)
      let file = rubymotion#buffer().alternate(a:count)
    endif
    if !empty(file)
...
function! s:readable_alternate_candidates(...) dict abort
  ...
  if a:0 && a:1
    ...
    elseif self.type_name('model-arb')
      ...
      if table_name == ''
        let table_name = rubymotion#pluralize(s:gsub(s:sub(fnamemodify(f,':r'),'.{-}<app/models/',''),'/','_'))
      endif
    ...
    elseif self.type_name('db-schema') && !empty(lastmethod)
      return ['app/models/' . rubymotion#singularize(lastmethod) . '.rb']
    endif
  endif
...
function! s:Extract(bang,...) range abort
  ...
  let rubymotion_root = rubymotion#app().path()
  let ext = expand("%:e")
  let file = s:sub(a:1,'%(/|^)\zs_\ze[^/]*$','')
  let first = a:firstline
  let last = a:lastline
  let range = first.",".last
  if rubymotion#buffer().type_name('view-layout')
    if rubymotionFilePath() =~ '\<app/views/layouts/application\>'
      let curdir = 'app/views/shared'
      if file !~ '/'
        let file = "shared/" .file
      endif
    else
      let curdir = s:sub(rubymotionFilePath(),'.*<app/views/layouts/(.*)%(\.\w*)$','app/views/\1')
    endif
  else
    let curdir = fnamemodify(rubymotionFilePath(),':h')
  endif
  let curdir = rubymotion_root.'/'.curdir
  ...
  if dir =~ '^/'
    let out = (rubymotion_root).dir."/_".fname
  elseif dir == "" || dir == "."
    let out = (curdir)."/_".fname
  elseif isdirectory(curdir."/".dir)
    let out = (curdir)."/".dir."/_".fname
  else
    let out = (rubymotion_root)."/app/views/".dir."/_".fname
  endif
...
function! s:RubyExtract(bang, root, before, name) range abort
  ...
  call append(a:firstline-1, repeat(' ', indent).'include '.rubymotion#camelize(a:name))
  let out = rubymotion#app().path(a:root, a:name . '.rb')
  if filereadable(out) && !a:bang
    return s:error('E13: File exists (add ! to override)')
  endif
  if !isdirectory(fnamemodify(out, ':h'))
  ...
  silent %delete_
  call setline(1, ['module '.rubymotion#camelize(a:name)] + a:before + content + ['end'])
endfunction
...
function! rubymotion#cache_clear(...)
  if exists('b:rubymotion_root')
    return call(rubymotion#app().cache.clear,a:000,rubymotion#app().cache)
  endif
endfunction
...
function! s:app_user_classes() dict
  if self.cache.needs("user_classes")
    ...
    let classes =
          ...
          \ self.relglob("lib/","**/*",".rb")
    call map(classes,'rubymotion#camelize(v:val)')
...
function! rubymotion#buffer_syntax()
  if !exists("g:rubymotion_no_syntax")
    let buffer = rubymotion#buffer()
    ...
    if &syntax == 'ruby'
      if !empty(special)
        exe 'syn match rubyrubymotionMethod "\<\%('.join(special, '\|').'\)"'
      endif
      if !empty(regular)
        exe 'syn keyword rubyrubymotionMethod '.join(regular, ' ')
      endif
      if buffer.type_name() == ''
        syn keyword rubyrubymotionMethod params request response session headers cookies flash
      endif
      if buffer.type_name() ==# 'model' || buffer.type_name('model-arb')
        syn keyword rubyrubymotionARMethod default_scope enum named_scope scope serialize store
        syn keyword rubyrubymotionARAssociationMethod belongs_to has_one has_many has_and_belongs_to_many composed_of accepts_nested_attributes_for
        syn keyword rubyrubymotionARCallbackMethod before_create before_destroy before_save before_update before_validation before_validation_on_create before_validation_on_update
        syn keyword rubyrubymotionARCallbackMethod after_create after_destroy after_save after_update after_validation after_validation_on_create after_validation_on_update
        syn keyword rubyrubymotionARCallbackMethod around_create around_destroy around_save around_update
        syn keyword rubyrubymotionARCallbackMethod after_commit after_find after_initialize after_rollback after_touch
        syn keyword rubyrubymotionARClassMethod attr_accessible attr_protected attr_readonly has_secure_password store_accessor
        syn keyword rubyrubymotionARValidationMethod validate validates validate_on_create validate_on_update validates_acceptance_of validates_associated validates_confirmation_of validates_each validates_exclusion_of validates_format_of validates_inclusion_of validates_length_of validates_numericality_of validates_presence_of validates_size_of validates_uniqueness_of validates_with
        syn keyword rubyrubymotionMethod logger
      endif
      if buffer.type_name('model-aro')
        syn keyword rubyrubymotionARMethod observe
      endif
      if buffer.type_name('mailer')
        syn keyword rubyrubymotionMethod logger url_for polymorphic_path polymorphic_url
        syn keyword rubyrubymotionRenderMethod mail render
        syn keyword rubyrubymotionControllerMethod attachments default helper helper_attr helper_method
      endif
      if buffer.type_name('helper','view')
        syn keyword rubyrubymotionViewMethod polymorphic_path polymorphic_url
        exe "syn keyword rubyrubymotionHelperMethod ".s:gsub(s:helpermethods(),'<%(content_for|select)\s+','')
        syn match rubyrubymotionHelperMethod '\<select\>\%(\s*{\|\s*do\>\|\s*(\=\s*&\)\@!'
        syn match rubyrubymotionHelperMethod '\<\%(content_for?\=\|current_page?\)'
        syn match rubyrubymotionViewMethod '\.\@<!\<\(h\|html_escape\|u\|url_encode\)\>'
        if buffer.type_name('view-partial')
          syn keyword rubyrubymotionMethod local_assigns
        endif
      elseif buffer.type_name('controller')
        syn keyword rubyrubymotionMethod params request response session headers cookies flash
        syn keyword rubyrubymotionRenderMethod render
        syn keyword rubyrubymotionMethod logger polymorphic_path polymorphic_url
        syn keyword rubyrubymotionControllerMethod helper helper_attr helper_method filter layout url_for serialize exempt_from_layout filter_parameter_logging hide_action cache_sweeper protect_from_forgery caches_page cache_page caches_action expire_page expire_action rescue_from
        syn keyword rubyrubymotionRenderMethod head redirect_to render_to_string respond_with
        syn match   rubyrubymotionRenderMethod '\<respond_to\>?\@!'
        syn keyword rubyrubymotionFilterMethod before_filter append_before_filter prepend_before_filter after_filter append_after_filter prepend_after_filter around_filter append_around_filter prepend_around_filter skip_before_filter skip_after_filter skip_filter before_action append_before_action prepend_before_action after_action append_after_action prepend_after_action around_action append_around_action prepend_around_action skip_before_action skip_after_action skip_action
        syn keyword rubyrubymotionFilterMethod verify
      endif
      if buffer.type_name('db-migration','db-schema')
        syn keyword rubyrubymotionMigrationMethod create_table change_table drop_table rename_table create_join_table drop_join_table
        syn keyword rubyrubymotionMigrationMethod add_column rename_column change_column change_column_default change_column_null remove_column remove_columns
        syn keyword rubyrubymotionMigrationMethod add_timestamps remove_timestamps
        syn keyword rubyrubymotionMigrationMethod add_reference remove_reference add_belongs_to remove_belongs_to
        syn keyword rubyrubymotionMigrationMethod add_index remove_index rename_index
        syn keyword rubyrubymotionMigrationMethod execute transaction reversible revert
      endif
      if buffer.type_name('test')
        if !empty(rubymotion#app().user_assertions())
          exe "syn keyword rubyrubymotionUserMethod ".join(rubymotion#app().user_assertions())
        endif
        syn keyword rubyrubymotionTestMethod refute refute_empty refute_equal refute_in_delta refute_in_epsilon refute_includes refute_instance_of refute_kind_of refute_match refute_nil refute_operator refute_predicate refute_respond_to refute_same
        syn keyword rubyrubymotionTestMethod add_assertion assert assert_block assert_equal assert_in_delta assert_instance_of assert_kind_of assert_match assert_nil assert_no_match assert_not_equal assert_not_nil assert_not_same assert_nothing_raised assert_nothing_thrown assert_operator assert_raise assert_respond_to assert_same assert_send assert_throws assert_recognizes assert_generates assert_routing flunk fixtures fixture_path use_transactional_fixtures use_instantiated_fixtures assert_difference assert_no_difference assert_valid
        syn keyword rubyrubymotionTestMethod test setup teardown
        if !buffer.type_name('test-unit')
          syn match   rubyrubymotionTestControllerMethod  '\.\@<!\<\%(get\|post\|put\|patch\|delete\|head\|process\|assigns\)\>'
          syn keyword rubyrubymotionTestControllerMethod get_via_redirect post_via_redirect put_via_redirect delete_via_redirect request_via_redirect
          syn keyword rubyrubymotionTestControllerMethod assert_response assert_redirected_to assert_template assert_recognizes assert_generates assert_routing assert_dom_equal assert_dom_not_equal assert_select assert_select_rjs assert_select_encoded assert_select_email assert_tag assert_no_tag
        endif
      elseif buffer.type_name('spec')
        syn keyword rubyrubymotionTestMethod describe context it its specify shared_context shared_examples_for it_should_behave_like it_behaves_like before after around subject fixtures controller_name helper_name scenario feature background
        syn match rubyrubymotionTestMethod '\<let\>!\='
        syn keyword rubyrubymotionTestMethod violated pending expect allow double instance_double mock mock_model stub_model
        syn match rubyrubymotionTestMethod '\.\@<!\<stub\>!\@!'
        if !buffer.type_name('spec-model')
          syn match   rubyrubymotionTestControllerMethod  '\.\@<!\<\%(get\|post\|put\|patch\|delete\|head\|process\|assigns\)\>'
          syn keyword rubyrubymotionTestControllerMethod  integrate_views render_views
          syn keyword rubyrubymotionMethod params request response session flash
          syn keyword rubyrubymotionMethod polymorphic_path polymorphic_url
        endif
      endif
      if buffer.type_name('task')
        syn match rubyrubymotionRakeMethod '^\s*\zs\%(task\|file\|namespace\|desc\|before\|after\|on\)\>\%(\s*=\)\@!'
      endif
      if buffer.type_name('config-routes')
        syn match rubyrubymotionMethod '\.\zs\%(connect\|named_route\)\>'
        syn keyword rubyrubymotionMethod match get put patch post delete redirect root resource resources collection member nested scope namespace controller constraints mount concern
      endif
      syn keyword rubyrubymotionMethod debugger
      syn keyword rubyrubymotionMethod alias_attribute alias_method_chain attr_accessor_with_default attr_internal attr_internal_accessor attr_internal_reader attr_internal_writer delegate mattr_accessor mattr_reader mattr_writer superclass_delegating_accessor superclass_delegating_reader superclass_delegating_writer
      ...
      syn keyword rubyrubymotionInclude require_dependency

      syn region  rubyString   matchgroup=rubyStringDelimiter start=+\%(:order\s*=>\s*\)\@<="+ skip=+\\\\\|\\"+ end=+"+ contains=@rubyStringSpecial,rubymotionOrderSpecial
      syn region  rubyString   matchgroup=rubyStringDelimiter start=+\%(:order\s*=>\s*\)\@<='+ skip=+\\\\\|\\'+ end=+'+ contains=@rubyStringSpecial,rubymotionOrderSpecial
      syn match   rubymotionOrderSpecial +\c\<\%(DE\|A\)SC\>+ contained
      syn region  rubyString   matchgroup=rubyStringDelimiter start=+\%(:conditions\s*=>\s*\[\s*\)\@<="+ skip=+\\\\\|\\"+ end=+"+ contains=@rubyStringSpecial,rubymotionConditionsSpecial
      syn region  rubyString   matchgroup=rubyStringDelimiter start=+\%(:conditions\s*=>\s*\[\s*\)\@<='+ skip=+\\\\\|\\'+ end=+'+ contains=@rubyStringSpecial,rubymotionConditionsSpecial
      syn match   rubymotionConditionsSpecial +?\|:\h\w*+ contained
      syn cluster rubyNotTop add=rubymotionOrderSpecial,rubymotionConditionsSpecial

    elseif &syntax =~# '^eruby\>' || &syntax == 'haml'
      let containedin = 'contained containedin=@'.&syntax.'rubymotionRegions'
      syn case match
      if !empty(special)
        exe 'syn match '.&syntax.'rubymotionMethod "\<\%('.join(special, '\|').'\)"' containedin
      endif
      if !empty(regular)
        exe 'syn keyword '.&syntax.'rubymotionMethod '.join(regular, ' ') containedin
      endif
      if &syntax == 'haml'
        exe 'syn cluster hamlrubymotionRegions contains=hamlRubyCodeIncluded,hamlRubyCode,hamlRubyHash,@hamlEmbeddedRuby,rubyInterpolation'
      else
        exe 'syn cluster erubyrubymotionRegions contains=erubyOneLiner,erubyBlock,erubyExpression,rubyInterpolation'
      endif
      exe 'syn keyword '.&syntax.'rubymotionHelperMethod '.s:gsub(s:helpermethods(),'<%(content_for|select)\s+','').' contained containedin=@'.&syntax.'rubymotionRegions'
      exe 'syn match '.&syntax.'rubymotionHelperMethod "\<select\>\%(\s*{\|\s*do\>\|\s*(\=\s*&\)\@!" contained containedin=@'.&syntax.'rubymotionRegions'
      exe 'syn match '.&syntax.'rubymotionHelperMethod "\<\%(content_for?\=\|current_page?\)" contained containedin=@'.&syntax.'rubymotionRegions'
      exe 'syn keyword '.&syntax.'rubymotionMethod debugger polymorphic_path polymorphic_url contained containedin=@'.&syntax.'rubymotionRegions'
      exe 'syn match '.&syntax.'rubymotionViewMethod "\.\@<!\<\(h\|html_escape\|u\|url_encode\)\>" contained containedin=@'.&syntax.'rubymotionRegions'
      if buffer.type_name('view-partial')
        exe 'syn keyword '.&syntax.'rubymotionMethod local_assigns contained containedin=@'.&syntax.'rubymotionRegions'
      endif
      exe 'syn keyword '.&syntax.'rubymotionRenderMethod render contained containedin=@'.&syntax.'rubymotionRegions'
      exe 'syn case match'
    elseif &syntax == "yaml"
      syn case match
      ...
      unlet g:main_syntax
      syn cluster yamlrubymotionRegions contains=yamlrubymotionOneLiner,yamlrubymotionBlock,yamlrubymotionExpression
      syn region  yamlrubymotionOneLiner   matchgroup=yamlrubymotionDelimiter start="^%%\@!" end="$"  contains=@rubyrubymotionTop      containedin=ALLBUT,@yamlrubymotionRegions,yamlrubymotionComment keepend oneline
      syn region  yamlrubymotionBlock      matchgroup=yamlrubymotionDelimiter start="<%%\@!" end="%>" contains=@rubyTop           containedin=ALLBUT,@yamlrubymotionRegions,yamlrubymotionComment
      syn region  yamlrubymotionExpression matchgroup=yamlrubymotionDelimiter start="<%="    end="%>" contains=@rubyTop           containedin=ALLBUT,@yamlrubymotionRegions,yamlrubymotionComment
      syn region  yamlrubymotionComment    matchgroup=yamlrubymotionDelimiter start="<%#"    end="%>" contains=rubyTodo,@Spell    containedin=ALLBUT,@yamlrubymotionRegions,yamlrubymotionComment keepend
      syn match yamlrubymotionMethod '\.\@<!\<\(h\|html_escape\|u\|url_encode\)\>' contained containedin=@yamlrubymotionRegions
...
function! s:HiDefaults()
  hi def link rubyrubymotionAPIMethod              rubyrubymotionMethod
  hi def link rubyrubymotionARAssociationMethod    rubyrubymotionARMethod
  hi def link rubyrubymotionARCallbackMethod       rubyrubymotionARMethod
  hi def link rubyrubymotionARClassMethod          rubyrubymotionARMethod
  hi def link rubyrubymotionARValidationMethod     rubyrubymotionARMethod
  hi def link rubyrubymotionARMethod               rubyrubymotionMethod
  hi def link rubyrubymotionRenderMethod           rubyrubymotionMethod
  hi def link rubyrubymotionHelperMethod           rubyrubymotionMethod
  hi def link rubyrubymotionViewMethod             rubyrubymotionMethod
  hi def link rubyrubymotionMigrationMethod        rubyrubymotionMethod
  hi def link rubyrubymotionControllerMethod       rubyrubymotionMethod
  hi def link rubyrubymotionFilterMethod           rubyrubymotionMethod
  hi def link rubyrubymotionTestControllerMethod   rubyrubymotionTestMethod
  hi def link rubyrubymotionTestMethod             rubyrubymotionMethod
  hi def link rubyrubymotionRakeMethod             rubyrubymotionMethod
  hi def link rubyrubymotionMethod                 rubymotionMethod
  hi def link rubyrubymotionInclude                rubyInclude
  hi def link rubyrubymotionUserClass              rubymotionUserClass
  hi def link rubyrubymotionUserMethod             rubymotionUserMethod
  hi def link erubyrubymotionHelperMethod          erubyrubymotionMethod
  hi def link erubyrubymotionViewMethod            erubyrubymotionMethod
  hi def link erubyrubymotionRenderMethod          erubyrubymotionMethod
  hi def link erubyrubymotionMethod                rubymotionMethod
  hi def link erubyrubymotionUserMethod            rubymotionUserMethod
  hi def link erubyrubymotionUserClass             rubymotionUserClass
  hi def link hamlrubymotionHelperMethod           hamlrubymotionMethod
  hi def link hamlrubymotionViewMethod             hamlrubymotionMethod
  hi def link hamlrubymotionRenderMethod           hamlrubymotionMethod
  hi def link hamlrubymotionMethod                 rubymotionMethod
  hi def link hamlrubymotionUserMethod             rubymotionUserMethod
  hi def link hamlrubymotionUserClass              rubymotionUserClass
  hi def link rubymotionUserMethod                 rubymotionMethod
  hi def link yamlrubymotionDelimiter              Delimiter
  hi def link yamlrubymotionMethod                 rubymotionMethod
  hi def link yamlrubymotionComment                Comment
  hi def link yamlrubymotionUserClass              rubymotionUserClass
  hi def link yamlrubymotionUserMethod             rubymotionUserMethod
  hi def link javascriptrubymotionFunction         rubymotionMethod
  hi def link rubymotionUserClass                  rubymotionClass
  hi def link rubymotionMethod                     Function
  hi def link rubymotionClass                      Type
  hi def link rubymotionOrderSpecial               rubymotionStringSpecial
  hi def link rubymotionConditionsSpecial          rubymotionStringSpecial
  hi def link rubymotionStringSpecial              Identifier
endfunction

function! rubymotion#log_syntax()
  if has('conceal')
    syn match rubymotionlogEscape      '\e\[[0-9;]*m' conceal
    syn match rubymotionlogEscapeMN    '\e\[[0-9;]*m' conceal nextgroup=rubymotionlogModelNum,rubymotionlogEscapeMN skipwhite contained
  else
    syn match rubymotionlogEscape      '\e\[[0-9;]*m'
    syn match rubymotionlogEscapeMN    '\e\[[0-9;]*m' nextgroup=rubymotionlogModelNum,rubymotionlogEscapeMN skipwhite contained
  endif
  syn match   rubymotionlogRender      '\%(^\s*\%(\e\[[0-9;]*m\)\=\)\@<=\%(Started\|Processing\|Rendering\|Rendered\|Redirected\|Completed\)\>'
  syn match   rubymotionlogComment     '^\s*# .*'
  syn match   rubymotionlogModel       '\%(^\s*\%(\e\[[0-9;]*m\)*\)\@<=\u\%(\w\|:\)* \%(Load\%( Including Associations\| IDs For Limited Eager Loading\)\=\|Columns\|Exists\|Count\|Create\|Update\|Destroy\|Delete all\)\>' skipwhite nextgroup=rubymotionlogModelNum,rubymotionlogEscapeMN
  syn match   rubymotionlogModel       '\%(^\s*\%(\e\[[0-9;]*m\)*\)\@<=\%(SQL\|CACHE\)\>' skipwhite nextgroup=rubymotionlogModelNum,rubymotionlogEscapeMN
  syn region  rubymotionlogModelNum    start='(' end=')' contains=rubymotionlogNumber contained skipwhite
  syn match   rubymotionlogNumber      '\<\d\+\>%'
  syn match   rubymotionlogNumber      '[ (]\@<=\<\d\+\.\d\+\>\.\@!'
  syn match   rubymotionlogNumber      '[ (]\@<=\<\d\+\.\d\+ms\>'
  syn region  rubymotionlogString      start='"' skip='\\"' end='"' oneline contained
  syn region  rubymotionlogHash        start='{' end='}' oneline contains=rubymotionlogHash,rubymotionlogString
  syn match   rubymotionlogIP          '\<\d\{1,3\}\%(\.\d\{1,3}\)\{3\}\>'
  syn match   rubymotionlogTimestamp   '\<\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d\>'
  syn match   rubymotionlogSessionID   '\<\x\{32\}\>'
  syn match   rubymotionlogIdentifier  '^\s*\%(Session ID\|Parameters\|Unpermitted parameters\)\ze:'
  syn match   rubymotionlogSuccess     '\<2\d\d \u[A-Za-z0-9 ]*\>'
  syn match   rubymotionlogRedirect    '\<3\d\d \u[A-Za-z0-9 ]*\>'
  syn match   rubymotionlogError       '\<[45]\d\d \u[A-Za-z0-9 ]*\>'
  syn match   rubymotionlogError       '^DEPRECATION WARNING\>'
  syn keyword rubymotionlogHTTP        OPTIONS GET HEAD POST PUT PATCH DELETE TRACE CONNECT
  hi def link rubymotionlogEscapeMN    rubymotionlogEscape
  hi def link rubymotionlogEscape      Ignore
  hi def link rubymotionlogComment     Comment
  hi def link rubymotionlogRender      Keyword
  hi def link rubymotionlogModel       Type
  hi def link rubymotionlogNumber      Number
  hi def link rubymotionlogString      String
  hi def link rubymotionlogSessionID   Constant
  hi def link rubymotionlogIdentifier  Identifier
  hi def link rubymotionlogRedirect    rubymotionlogSuccess
  hi def link rubymotionlogSuccess     Special
  hi def link rubymotionlogError       Error
  hi def link rubymotionlogHTTP        Special
endfunction

function! rubymotion#log_setup() abort
...
function! s:BufMappings()
  nnoremap <buffer> <silent> <Plug>rubymotionFind       :<C-U>call <SID>Find(v:count1,'E')<CR>
  nnoremap <buffer> <silent> <Plug>rubymotionSplitFind  :<C-U>call <SID>Find(v:count1,'S')<CR>
  nnoremap <buffer> <silent> <Plug>rubymotionVSplitFind :<C-U>call <SID>Find(v:count1,'V')<CR>
  nnoremap <buffer> <silent> <Plug>rubymotionTabFind    :<C-U>call <SID>Find(v:count1,'T')<CR>
  if !hasmapto("<Plug>rubymotionFind")
    nmap <buffer> gf              <Plug>rubymotionFind
  endif
  if !hasmapto("<Plug>rubymotionSplitFind")
    nmap <buffer> <C-W>f          <Plug>rubymotionSplitFind
  endif
  if !hasmapto("<Plug>rubymotionTabFind")
    nmap <buffer> <C-W>gf         <Plug>rubymotionTabFind
  endif
endfunction
...
function! s:BufDatabase(level, ...)
  if exists("s:lock_database") || !exists('g:loaded_dbext') || !exists('b:rubymotion_root')
    return
  endif
  let self = rubymotion#app()
  if a:level > 1
    call self.cache.clear('dbext_settings')
  elseif exists('g:rubymotion_no_dbext')
    return
  endif
  if (a:0 && !empty(a:1))
    let env = a:1
...
if !exists('g:rubymotion_no_abbreviations') && type(get(g:, 'rubymotion_abbreviations', {})) == type(0)
  call s:error('Use rubymotion_no_abbreviations not rubymotion_abbreviations to disable abbreviations')
  let g:rubymotion_no_abbreviations = 1
endif

function! s:BufAbbreviations()
  " Some of these were cherry picked from the TextMate snippets
  if !exists('g:rubymotion_no_abbreviations')
    let buffer = rubymotion#buffer()
    ...
    " )))))
    for pairs in
          \ items(type(get(g:, 'rubymotion_abbreviations', 0)) == type({}) ? g:rubymotion_abbreviations : {})
      call call(function(s:sid.'Abbrev'), [0, pairs[0]] + s:split(pairs[1]))
    endfor
    for hash in reverse(rubymotion#buffer().projected('abbreviations'))
...
function! s:Abbrev(bang,...) abort
  if !exists("b:rubymotion_abbreviations")
    let b:rubymotion_abbreviations = {}
  endif
  if a:0 > 3 || (a:bang && (a:0 != 1))
    return s:error("Rabbrev: invalid arguments")
  endif
  if a:0 == 0
    for key in sort(keys(b:rubymotion_abbreviations))
      echo key . join(b:rubymotion_abbreviations[key],"\t")
    endfor
    return
  endif
  let lhs = a:1
  let root = s:sub(lhs,'%(::|\(|\[)$','')
  if a:bang
    if has_key(b:rubymotion_abbreviations,root)
      call remove(b:rubymotion_abbreviations,root)
    endif
  ...
  if lhs =~ '($'
    let b:rubymotion_abbreviations[root] = ["(", rhs . (a:0 > 2 ? "\t".a:3 : "")]
    if a:0 > 2
  ...
  let b:rubymotion_abbreviations[root] = [matchstr(lhs,'\W*$'),rhs]
endfunction

" }}}1
" Projections {{{1

function! rubymotion#json_parse(string) abort
...
function! s:app_projections() dict abort
  let dict = {}
  call s:combine_projections(dict, get(g:, 'rubymotion_projections', ''), {'check': 1})
  for gem in keys(get(g:, 'rubymotion_gem_projections', {}))
    if self.has_gem(gem)
      call s:combine_projections(dict, g:rubymotion_gem_projections[gem])
    endif
  endfor
  ...
  if !empty(gem_path)
    if !has_key(s:projections_for_gems, gem_path)
      let gem_projections = {}
      for path in ['lib/', 'lib/rubymotion/']
        for file in findfile(path.'projections.json', gem_path, -1)
          try
            call s:combine_projections(gem_projections, rubymotion#json_parse(readfile(self.path(file))))
          catch
          endtry
        endfor
      endfor
  ...
  if self.cache.needs('projections')
    ...
    if self.has_path('config/projections.json')
      try
        let projections = rubymotion#json_parse(readfile(self.path('config/projections.json')))
...
function! s:transformations.plural(input, o) abort
  return rubymotion#pluralize(a:input)
endfunction

function! s:transformations.singular(input, o) abort
  return rubymotion#singularize(a:input)
endfunction
  ...
  for pattern in reverse(sort(filter(keys(all), 'v:val =~# "^[^*{}]*\\*[^*{}]*$"'), s:function('rubymotion#lencmp')))
    ...
    if s:startswith(f, prefix) && s:endswith(f, suffix)
      ...
      let ph = extend({
            ...
            \ 'S': rubymotion#camelize(root),
            \ 'h': toupper(root[0]) . tr(rubymotion#underscore(root), '_', ' ')[1:-1],
            \ 'p': rubymotion#pluralize(root),
            \ 'i': rubymotion#singularize(root),
            \ '%': '%'}, a:0 ? a:1 : {})
      if suffix =~# '\.js\>'
...
function! s:SetBasePath() abort
  ...
  let path = ['lib', 'vendor']
  let path += get(g:, 'rubymotion_path_additions', [])
  let path += get(g:, 'rubymotion_path', [])
  ...
  let path += ['vendor/plugins/*/lib', 'vendor/plugins/*/test', 'vendor/rubymotion/*/lib', 'vendor/rubymotion/*/test']
  call map(path, 'rubymotion#app().path(v:val)')
...
function! rubymotion#buffer_setup() abort
  if !exists('b:rubymotion_root')
    return ''
  endif
  let self = rubymotion#buffer()
  let b:rubymotion_cached_file_type = self.calculate_file_type()
  call s:BufMappings()
  call s:BufCommands()
  if !empty(findfile('macros/rubymotion.vim', escape(&runtimepath, ' ')))
    runtime! macros/rubymotion.vim
  endif
  silent doautocmd User rubymotion
  ...
  call self.setvar('&includeexpr','rubymotionIncludeexpr()')
  ...
  if ft =~# '^ruby\>'
    ...
    if exists(':UltiSnipsAddFiletypes') == 2
      UltiSnipsAddFiletypes rubymotion
    elseif exists(':SnipMateLoadScope') == 2
      SnipMateLoadScope rubymotion
    endif
...
augroup rubymotionPluginAuto
  autocmd!
  autocmd User BufEnterrubymotion call s:RefreshBuffer()
  autocmd User BufEnterrubymotion call s:resetomnicomplete()
  autocmd User BufEnterrubymotion call s:BufDatabase(-1)
  autocmd User dbextPreConnection call s:BufDatabase(1)
  autocmd BufWritePost */config/database.yml      call rubymotion#cache_clear("dbext_settings")
  autocmd BufWritePost */config/projections.json  call rubymotion#cache_clear("projections")
  autocmd BufWritePost */test/test_helper.rb      call rubymotion#cache_clear("user_assertions")
  autocmd BufWritePost */config/routes.rb         call rubymotion#cache_clear("named_routes")
  autocmd BufWritePost */config/application.rb    call rubymotion#cache_clear("default_locale")
  autocmd BufWritePost */config/application.rb    call rubymotion#cache_clear("stylesheet_suffix")
  autocmd BufWritePost */config/environments/*.rb call rubymotion#cache_clear("environments")
  autocmd BufWritePost */tasks/**.rake            call rubymotion#cache_clear("rake_tasks")
  autocmd BufWritePost */generators/**            call rubymotion#cache_clear("generators")
augroup END
